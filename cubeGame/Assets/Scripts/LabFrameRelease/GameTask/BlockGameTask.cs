...
public class BlockGameTask : TaskBase
{
    ...
    public void User_MissingOneCube()
    {
        if (RandomQuestion == 1)
        {
            GameObject.Find("Q1BlueCube(Clone)").GetComponent<BoxCollider>().enabled = false;
            GameObject.Find("Q1BlueCube(Clone)").GetComponent<MeshRenderer>().enabled = false;
        }
        else if (RandomQuestion == 2)
        {
